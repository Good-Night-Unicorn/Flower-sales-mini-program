...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','xianhuafenlei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','xianhuafenlei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('xianhuafenlei', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussxianhuafenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('xianhuafenlei', this.detail);
				this.$utils.msg('审核成功');
