<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.shangpubianhao" type="text" placeholder="商铺编号" ></input>
			</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.shangpubianhao="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shangpubianhao){
			...
			async search(){
				...
				if(this.searchForm.shangpubianhao){
					searchForm['shangpubianhao'] = '%' + this.searchForm.shangpubianhao + '%'
				}
				if(this.searchForm.shangpumingcheng){
